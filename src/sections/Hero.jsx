  ...
  return (
    <section id='home' className='w-full flex xl:flex-row flex-col justify-center min-h-screen gap-10 max-container'>
      <div className="relative xl:w-2/5 flex flex-col justify-center items-start w-full max-xl:padding-x pt-28">
        ...
        <p className='font-montserrat text-slate-gray sm:max-w-sm text-lg leading-8 mb-14 mt-6'>Discover stylish Nike arrivals, quality comfort, and innovation for
          your active life.</p>
      ...
      <div className='flex sm:gap-6 gap-4 absolute -bottom-[5%] sm: left-[10%] max-sm:px-6'>
        {shoes.map((item, index) => (
          <div key={index}>
            <ShoesCard
              imgUrl = {item}
              changeBigShoesImage = {(item) => setBigShoesImage(item)}
              bigShoesImage = {bigShoesImage}
            />
          </div>
        ))}
      </div>
